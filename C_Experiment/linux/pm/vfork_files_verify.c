...
long myIoctl (struct file * fdd, unsigned int pid, unsigned long j)
{
	...
	file = task -> files;
	printk(KERN_INFO "Fd : %lu\n", j);
	printk(KERN_INFO "IN myIoctl\n\n");
	printk(KERN_INFO "files address : %p\n", file);
	/* (For vfork)Seprate address for child files and parent files 
	   child does not share parent's files
	*/
//	printk(KERN_INFO "Files -> fdt : %p\n", file -> fdt);
	printk(KERN_INFO "Files -> fdt -> fd : %p\n",(file -> fdt -> fd[j]));
	return 0; 
}
