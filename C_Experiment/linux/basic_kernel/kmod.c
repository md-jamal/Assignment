...
long myIoctl(struct file *fp, unsigned int pid, unsigned long b);
...
long myIoctl(struct file *fp, unsigned int pid, unsigned long b)
{
	...
	task =	find_task_by_vpid(pid);			//current	
	printk("task_struct pid is:%d\n", task -> pid);
	printk ("pid from application : %u\nKernel module name : %s\n", pid, mod -> name);
