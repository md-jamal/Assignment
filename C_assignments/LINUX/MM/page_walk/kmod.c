...
long myUnlocked_ioctl(struct file *fp, unsigned int pid, unsigned long g_addr)
{
	int i, j;
	...
	unsigned long *vaddr, pt_paddr, pt_vaddr;
	...
	pg_addr = m_ptr->pgd[(g_addr>>22)].pgd;
		
	if (((pg_addr = ((pg_addr>> 12)<<12)) <= 0x38000000)) {
		vaddr = pg_addr + 0xc0000000;
	} else {
		vaddr = (unsigned long *) kmap((mem_map + (pg_addr >> 12)));
	}

	
	pt_paddr = (unsigned long)(vaddr[((g_addr << 10)>>22)]);
	...
	*((unsigned long *)(pt_vaddr + ((g_addr << 20)>>20))) = 150;
